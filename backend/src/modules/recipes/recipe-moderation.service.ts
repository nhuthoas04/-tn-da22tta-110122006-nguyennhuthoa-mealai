...
export class RecipeModerationService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (isPlaceholder) {
      this.logger.warn('GEMINI_API_KEY is not defined or is a placeholder. Recipe AI Moderation will run in fallback/static mode.');
      this.genAI = null;
    } else {
      try {
        this.genAI = new GoogleGenerativeAI(apiKey);
      } catch (err: any) {
        this.logger.error('Failed to initialize Gemini AI for Moderation:', err.message);
        this.genAI = null;
      }
    }
  }
  ...
  async auditRecipe(recipeId: string): Promise<RecipeModerationAudit> {
    ...
    const ingredientNames = recipe.recipeIngredients?.map(ri => ri.ingredient?.name?.toLowerCase() || '') || [];
    ...
    const keywordsToCheck = ['tỏi', 'hành', 'tiêu', 'ớt', 'muối', 'đường', 'mắm', 'dầu', 'bơ', 'sữa', 'trứng', 'thịt', 'cá', 'tôm', 'gà'];
    for (const kw of keywordsToCheck) {
      if (stepWords.includes(kw) && !ingredientNames.some(ing => ing.includes(kw))) {
        missingIngredients.push(`Thiếu nguyên liệu có từ khóa "${kw}" mặc dù có nhắc tới ở các bước chế biến.`);
      }
    }

    if (recipe.steps.length < 2) {
      missingSteps.push('Các bước chế biến quá ngắn (ít hơn 2 bước). Hãy điền chi tiết hơn.');
      qualityScore -= 20;
    }

    if (ingredientNames.length < 2) {
      missingIngredients.push('Danh sách nguyên liệu quá nghèo nàn (ít hơn 2 loại nguyên liệu).');
      qualityScore -= 20;
    }

    let rawFeedback = 'Kiểm thử tĩnh thành công.';
    let nutritionValidityNotes = 'Calo tĩnh có vẻ hợp lý dựa trên ước lượng nguyên liệu.';
    ...
    if (this.genAI) {
      try {
        ...
        if (aiResult) {
          if (aiResult.missingIngredients && Array.isArray(aiResult.missingIngredients)) {
          ...
          nutritionValidityNotes = aiResult.nutritionValidityNotes || nutritionValidityNotes;
          rawFeedback = aiResult.feedback || rawFeedback;
        }
      } catch (err: any) {
        this.logger.error('Gemini AI recipe moderation failed, relying on static rules:', err.message);
    ...
    audit.isApprovedByAI = audit.qualityScore >= 70 && audit.missingIngredients.length === 0 && !audit.isDuplicateDetected;
  ...
  async getAuditByRecipeId(recipeId: string): Promise<RecipeModerationAudit | null> {
