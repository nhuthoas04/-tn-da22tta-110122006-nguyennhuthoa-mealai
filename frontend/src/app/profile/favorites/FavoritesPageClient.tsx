...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    favorites.forEach((recipe) => console.log(recipe.imageUrl));
  }, [favorites]);
  ...
  const loadFavorites = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Không thể tải danh sách yêu thích');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUnfavorite = async (id: string) => {
    try {
      ...
      toast.error(err.response?.data?.message || 'Không thể bỏ yêu thích');
    }
  };
    ...
    <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {favorites.map((fav) => (
                  <div key={fav.id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm flex flex-col justify-between hover:shadow-md transition">
                    <div className="relative h-36 sm:h-40 bg-gray-100 shrink-0">
                      <RecipeImage
                        src={fav.imageUrl}
                        alt={fav.name}
                    ...
                    <div className="p-3 sm:p-4 flex-1 flex flex-col justify-between">
                      <div className="space-y-1">
                        <span className="inline-block text-[9px] sm:text-[10px] uppercase font-bold tracking-wider text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">
                          {fav.cuisineRegion || 'Ẩm thực'}
                        </span>
                        <h3 className="font-bold text-gray-955 text-sm sm:text-base line-clamp-1 hover:text-emerald-700">
                          <Link href={`/recipes/${fav.id}`}>{fav.name}</Link>
                        </h3>
                        <div className="flex gap-3 text-[10px] sm:text-xs text-gray-500 mt-2">
                      ...
                      {/* Actions */}
                      <div className="grid grid-cols-3 gap-1.5 mt-4 pt-3 border-t border-gray-100 shrink-0">
                        <Link 
                          href={`/recipes/${fav.id}`}
                          className="flex items-center justify-center gap-1 py-2 border border-gray-200 rounded-xl text-[10px] sm:text-xs font-semibold text-gray-700 bg-gray-50 hover:bg-gray-100 transition whitespace-nowrap"
                        >
                          <HiEye /> Chi tiết
                        </Link>
                        <button 
                          ...
                          className="flex items-center justify-center gap-1 py-2 border border-emerald-200 rounded-xl text-[10px] sm:text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100/70 transition whitespace-nowrap"
                        >
                          <HiCalendar /> Thực đơn
                        </button>
                        <button 
                          onClick={() => handleAddToShoppingList(fav.id)}
                          className="flex items-center justify-center gap-1 py-2 border border-blue-200 rounded-xl text-[10px] sm:text-xs font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100/70 transition whitespace-nowrap"
                        >
      ...
      {plannerModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl flex flex-col max-h-[90vh] border border-gray-100 animate-scale-up">
            <div className="mb-4">
            ...
            <div className="space-y-3 overflow-y-auto flex-1 pr-1 pb-2">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 outline-none cursor-pointer"
                >
                  {[1, 2, 3, 4, 5, 6, 7].map(d => (
